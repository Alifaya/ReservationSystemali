	...
	public void showCLIMenu() {
		...
		while(true) {
			...
			/*	System.out.println("Enter person name, id, age (press Enter after each):");
			    ...
			    // Age input validation loop
			    while (true) {
			        String ageInput = sc.next();
			        try {
			            age = Integer.parseInt(ageInput);
			            break; // Break the loop if age is a valid integer
			        } catch (NumberFormatException e) {
			            System.out.println("Wrong age. Please, enter a valid integer for age");
			        }
			    }

			    // Create a new Person object with the provided name, ID, and age
			     p = new Person(name, id, age);
			    people.addPerson(p);

			    System.out.println("Enter room name:");
			    String roomName = sc.next();
			     r = new LabRoom(roomName);

			    // Date input validation loop
System.out.println("Enter Date in yyyy-mm-dd:");
			    while (true) {
			        String dateInput = sc.next();
			        try {
			            l = LocalDate.parse(dateInput);
			            break; // Break the loop if the date format is correct
			        } catch (DateTimeParseException e) {
			            System.out.println("Invalid date format. Please re-enter the date in yyyy-mm-dd format.");
			        }
			    }
			    ...
			    while (true) {
			        if (sc.hasNextInt()) {
			            ...
			            if (t >= 1 && t <= 24) {
			                break; // Break the loop if the hour is valid
			            } else {
			    ...
			    // Reserve the block with the given details
			     st = this.reserveBlock(new ReserveBlock(p, l, t, r));
			    break;
			...
			case "7":
				...
				people.addPerson(userName, userID, userAge);
				break;
	...
	public boolean loadDataFromFile() {
	    ...
	    // Create a File object for the sessions file
	    File sessionsFile = new File(SESSIONS_FILE);

	    if (!sessionsFile.exists()) {
	        System.out.println("For Sessions, File Not Found to load!");
	        return false;
	    }
	    ...
	    try (ObjectInputStream o = new ObjectInputStream(new FileInputStream(sessionsFile))) {
	        ...
	        System.out.println("Class not found in load file! - " + e.getMessage());
	    } catch (Exception e) {
	        // General exception handling
	        System.out.println("Unknown error in load file! - " + e.getMessage());
	        e.printStackTrace();
	    }
	...
	public boolean saveDataToFile() {
		if(KkuSystem.reservations.isEmpty())
		{
			...
			try {
				o2 = new ObjectOutputStream(new FileOutputStream(SESSIONS_FILE));
    ...
    @Override
    public List<Person> findPeopleReservedBlock(ReserveBlock tr_Block) {
        // Initialize a list to store the people who have reserved the target block
        List<Person> peopleReserved = new ArrayList<>();
        for (ReserveBlock reservation : reservations) {
            if (reservation.equals(tr_Block)) {
                peopleReserved.add(reservation.getBy());
            }
        }
        return peopleReserved;
    }
